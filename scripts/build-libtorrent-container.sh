...
./configure --host=x86_64-unknown-freebsd12.0 --prefix=/src/build/oo \
    --with-scenetpoll --without-epoll --without-kqueue \
    --disable-shared --enable-static \
    --disable-debug \
    CC="$CC" CXX="$CXX" AR="$AR" RANLIB="$RANLIB" \
    CFLAGS="$CFLAGS" CXXFLAGS="$CXXFLAGS" LDFLAGS="$LDFLAGS"
make -j"$(nproc)" || make -j2
# compile PS4 shim (real SHA-1/RC4) into the static lib
ps4-clang -O2 -D_GNU_SOURCE -include stdint.h -I/src/shim -c /src/shim/ps4_crypto.c -o shim_ps4_crypto.o
llvm-ar r src/libtorrent.a shim_ps4_crypto.o
make install
